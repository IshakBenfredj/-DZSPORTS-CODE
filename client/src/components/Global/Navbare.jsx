...
return (
    <>
    <nav className='fixed left-0 right-0 z-50 overflow-hidden' style={navbarStyle}>
        <div className="container nav w-full flex items-center">
            <div className="logo flex items-center">
                <Link to='/dzsports'><img src={scrollPosition < 200 && isHome ? logoWhite : logoMain} className='logoImg hidden md:block' alt="DZSPORTS LOGO" onClick={scrollToTop} /></Link>
                <Link to='/dzsports'><img src={scrollPosition < 200 && isHome ? logoWhiteResp : logoMainResp} className='logoImg block md:hidden' alt="DZSPORTS LOGO" onClick={scrollToTop} /></Link>
            </div>
